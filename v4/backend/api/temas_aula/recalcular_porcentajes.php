...
// de la copia de aula. Espejo de api/temas/recalcular_porcentajes.php.
//
...
// la evaluación anual (temas_aula.peso_evaluacion). El peso de cada unidad
// se reparte entre los RA que intervienen en ella, en proporción a los
// criterios de evaluación (CE) de cada RA en esa unidad (criterios_temas_aula).
...
try {
    ...
    $pesos = array();
    foreach ($db->fetchAll(
        "SELECT id, peso_evaluacion FROM temas_aula
          WHERE idMateria = ? AND idGrupo = ? AND idProfesor = ?",
        $idMateria, $idGrupo, $idProfesor) as $fila) {
    ...
    foreach ($db->fetchAll(
        "SELECT idRA, idTema, COUNT(*) AS n
           FROM criterios_temas_aula
          WHERE idMateria = ? AND idGrupo = ? AND idProfesor = ?
          GROUP BY idRA, idTema", $idMateria, $idGrupo, $idProfesor) as $fila) {
        $ra   = intval($fila['idRA']);
    ...
    $porcentajes = array();
    foreach ($db->fetchAll(
        "SELECT id FROM resultados_aprendizaje_aula
          WHERE idMateria = ? AND idGrupo = ? AND idProfesor = ? ORDER BY orden",
        $idMateria, $idGrupo, $idProfesor) as $fila) {
        $raId = intval($fila['id']);
        $total = 0;
        if (isset($ceRA[$raId])) {
    ...
    foreach ($porcentajes as $p) {
        $db->execute(
            "UPDATE resultados_aprendizaje_aula
               SET porcentaje_evaluacion = ?
             WHERE id = ?",
            $p['porcentaje'], $p['id']);
    }
