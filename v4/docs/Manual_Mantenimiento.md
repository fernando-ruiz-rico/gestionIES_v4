...
> **Nota para v4:** este documento conserva la descripción funcional y del modelo de datos histórico. La estructura técnica cambió: PHP está exclusivamente en `backend/`, el cliente estático Vue 3 en `frontend/` y jQuery/jQuery UI ya no forman parte de la aplicación. Para rutas, despliegue y flujo de ejecución debe consultarse `ARQUITECTURA_V4.md` y el `README.md` principal.
