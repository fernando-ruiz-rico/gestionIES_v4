...
$idDepartamento = isset($_GET['idDepartamento']) ? (int)$_GET['idDepartamento'] : 1;
            ...
            foreach ($profesores as $profesor) {
                ...
                foreach ($grupos as $grupo) {
                    ...
                    if ($nombreArchivo !== null) {
                        echo "pdf_programaciones_aula.php?idMateria={$materia['id']}&idGrupo={$grupo['id']}&idProfesor={$profesor['id']}\tCiclos_formativos/$nombreCicloDir/" . limpiarCurso($curso['nombre']) . "/$nombreArchivo\n";
                    }
                }
            }
        }
...
foreach ($ciclosPCCF as $ciclo) {
    ...
    echo "pdf_pccf.php?idCiclo={$ciclo['id']}\tPCCF/$nombrePdf\n";
}
...
foreach ($materiasOtros as $materia) {
    ...
    foreach ($profesores as $profesor) {
        ...
        foreach ($grupos as $grupo) {
            ...
            if ($nombreArchivo !== null) {
                echo "pdf_programaciones_aula.php?idMateria={$materia['id']}&idGrupo={$grupo['id']}&idProfesor={$profesor['id']}\tOtros_estudios_ESO_Bachillerato/Programaciones_de_aula/$nombreArchivo\n";
            }
        }
    }
}
